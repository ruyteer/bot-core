...
const STATEMENTS: string[] = [
  ...
  // 0025_lead_progress_unique_and_update_dedupe.sql — auditoria do runner do
  // funil de FLUXO (Achado 1): `lead_progress` sem restrição de unicidade +
  // pubsub de update do Telegram at-least-once (e reentrega de webhook do
  // Telegram) inseriam DOIS progressos pro mesmo lead quando o mesmo update
  // era processado duas vezes.

  // Dedupe de update do Telegram por bot. `update_id` é sequencial só DENTRO
  // do bot, daí a chave composta. Criada ANTES do bloco de fusão de
  // lead_progress abaixo (sem dependência entre as duas — só pra o boot não
  // ficar com metade do trabalho feito se um statement no meio falhar).
  ...
  // saída do funil. Nunca lido pelo runner — `status` continua regendo o
  // comportamento.
  `ALTER TABLE "lead_progress" ADD COLUMN IF NOT EXISTS "stall_reason" text`,
  ...
  // ainda setado (não terminou, não está no meio de um delay) vence; empate
  // por updated_at mais recente. Delays pendentes e pagamentos das linhas
  // descartadas são REAPONTADOS pro vencedor (não perdem o timeout nem o
  // contexto de retomada) antes de a linha perdedora ser apagada.
  //
  // Dedupe + CREATE UNIQUE INDEX andam num ÚNICO DO $$, com um LOCK TABLE
  // antes: sem o lock, entre o SELECT que monta a fusão e o CREATE INDEX no
  // fim deste MESMO statement, uma réplica antiga do runner (rodando o
  // binário anterior ao upsert atômico) podia inserir um progresso duplicado
  // no meio do caminho — o índice falharia ao criar, e daí em diante TODO
  // onConflict do código novo passaria a lançar unique_violation em vez de
  // resolver o upsert. SHARE ROW EXCLUSIVE deixa leitura concorrente livre e
  // só bloqueia escrita em lead_progress até o commit deste bloco.
  //
  // Este statement inteiro roda TODO BOOT (ensureSchema reaplica sempre) — o
  // `IF NOT EXISTS (... pg_indexes ...)` faz tudo virar no-op (sem nem tomar
  // o lock) assim que o índice existir, em vez de refazer o trabalho pra
  // sempre conforme a tabela cresce.
  `DO $$
   BEGIN
     IF NOT EXISTS (
